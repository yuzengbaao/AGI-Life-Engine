...
class NodeLayer:
    """节点分层定义"""
    CORE = "core"           # 核心层：当前活跃（最近5分钟）
    RECENT = "recent"       # 中间层：近期使用（1小时内）
    HISTORY = "history"     # 外围层：历史记忆（更早）
...
class FileLock:
    ...
    def acquire(self) -> bool:
        ...
        while time.time() - start_time < self.timeout:
            try:
                # 灏濊瘯鍒涘缓閿佹枃浠讹紙鍘熷瓙鎿嶄綔锛?
                # 浣跨敤 O_EXCL 鍜?O_CREAT 鏍囧織纭繚鍘熷瓙鎬?
                ...
                self.acquired = True
                # logger.debug(f"鉁?File lock acquired: {self.lock_file}")
                return True

            except FileExistsError:
                # 閿佹枃浠跺凡瀛樺湪锛屾鏌ユ槸鍚︿负杩囨湡閿?
                try:
                    lock_content = ""
                    try:
                        # 璇诲彇閿佹枃浠跺唴瀹?
                        ...
                        # 璇诲彇澶辫触锛堝彲鑳芥鍦ㄥ啓鍏ユ垨鏉冮檺闂锛夛紝璺宠繃鏈妫€鏌?
                    ...
                    if len(parts) >= 3:
                        try:
                            ...
                            # 濡傛灉閿佽秴杩?0鍒嗛挓锛岃涓烘槸杩囨湡閿佸苟鍒犻櫎
                            if time.time() - lock_time > 600:
                                logger.warning(f"鈿狅笍 Removing stale lock file (timeout): {self.lock_file}")
                                try:
                    ...
                    # 2. 妫€鏌ラ攣杩涚▼鏄惁杩樺湪杩愯锛堜粎Unix锛?
                    try:
                        if not parts or not parts[0]:
                            raise ValueError("Empty PID")
                            
                        pid = int(parts[0])
                        os.kill(pid, 0)  # 妫€鏌ヨ繘绋嬫槸鍚﹀瓨鍦?
                    except (ProcessLookupError, ValueError, IndexError):
                        # 杩涚▼涓嶅瓨鍦ㄦ垨PID鏃犳晥锛屽彲浠ュ垊闄ら攣
                        # logger.warning(f"鈿狅笍 Removing orphaned lock file: {self.lock_file}")
                        try:
                            self.lock_file.unlink()
                        except OSError:
                            pass # 鍒犻櫎澶辫触锛堝彲鑳借鍗犵敤锛夛紝蹇界暐
                        continue
        ...
        logger.warning(f"鈴憋笍 Timeout acquiring lock: {self.lock_file}")
        return False
        ...
        if self.acquired and self.lock_file.exists():
            try:
                ...
                if lock_content.startswith(str(os.getpid())):
                    self.lock_file.unlink()
                    logger.debug(f"🔓 File lock released: {self.lock_file}")
                else:
                    logger.warning(f"⚠️ Attempted to release another process's lock")
...
class KnowledgeGraphExporter:
    ...
    def __init__(self,
        ...
        logger.info(f"📊 知识图谱导出器初始化完成 (v2.0 - 增强版)")
        logger.info(f"   - 输出目录: {self.output_dir}")
        logger.info(f"   - 导出间隔: {self.export_interval}秒")
        logger.info(f"   - 主文件: {self.main_file}")
        logger.info(f"   - 已有节点: {len(self.node_index)}")
    ...
    def _load_existing_data(self):
        ...
        try:
            if self.main_file.exists():
                ...
                logger.info(f"✅ 已加载 {len(self.node_index)} 个历史节点, {len(self.link_index)} 条链接")
        ...
        with self.lock:
            try:
                ...
                with FileLock(str(lock_file), timeout=60.0, poll_interval=0.2):
                    # 鍘熷瓙鍖栧啓鍏ワ紙鍏堝啓涓存椂鏂囦欢锛屽啀閲嶅懡鍚嶏級
                    temp_file = self.main_file.with_suffix('.tmp')
                    with open(temp_file, 'w', encoding='utf-8') as f:
                        json.dump(export_data, f, indent=2, ensure_ascii=False)

                    # 🔧 [FIX 2026-01-27] Windows兼容的原子重命名（增强重试机制处理文件占用）
                    max_retries = 5
                    retry_delay = 0.5  # 秒
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            logger.debug(f"✅ 原子重命名成功 (尝试 {attempt + 1}/{max_retries})")
                            export_success = True
                            break
                            ...
                            try:
                                ...
                                temp_file.unlink()
                                logger.debug(f"✅ 复制成功 (尝试 {attempt + 1}/{max_retries})")
                                export_success = True
                                break
                            except Exception as copy_err:
                                if attempt < max_retries - 1:
                                    logger.warning(f"⚠️ 文件操作失败 (尝试 {attempt + 1}/{max_retries}): {copy_err}")
                                    logger.debug(f"   等待 {retry_delay}秒 后重试...")
                                    time.sleep(retry_delay)
                                else:
                                    logger.error(f"❌ 文件操作彻底失败 (已重试 {max_retries} 次): {copy_err}")
                                    # 保留临时文件，下次导出时会覆盖

                    if not export_success:
                        return False  # 导出失败，直接返回
                ...
                logger.debug(f"✅ 知识图谱已导出: {self.stats['total_nodes']}个节点 "
                           f"(核心:{self.stats['core_nodes']} "
                           f"近期:{self.stats['recent_nodes']} "
                           f"历史:{self.stats['history_nodes']}) "
                           f", {self.stats['total_links']}条边")
    ...
    def update_from_agi_system(self, agi_engine) -> bool:
        ...
        with self.lock:
            try:
                ...
                logger.debug(f"📊 知识图谱数据已更新: "
                           f"总计{len(self.node_index)}个节点 "
                           f"(核心:{len(core_nodes)}), "
                           f"{len(links)}条边")
    ...
    def _collect_historical_nodes(self, agi_engine, current_time: datetime):
        ...
        if hasattr(agi_engine, 'evolution_controller') and agi_engine.evolution_controller:
            ...
            if hasattr(evo, 'value_network'):
                ...
                if hasattr(vn, 'decision_history'):
                    for decision in vn.decision_history[-100:]:  # 最近100个决策
        ...
        if hasattr(agi_engine, 'neural_memory') and agi_engine.neural_memory:
            ...
            try:
                        ...
                        for i, memory_id in enumerate(result['ids']):
                            ...
                            try:
                                if isinstance(created_str, str):
                                    ...
                                    created_time = current_time - timedelta(hours=24)  # 默认24小时前
                            except:
                ...
                logger.debug(f"从NeuralMemory收集节点失败: {e}")

    def _collect_conversation_nodes(self, current_time: datetime):
        """
        🆕 从 user_intents.jsonl 收集对话记忆节点
        将用户问题、疑问和系统响应映射为知识图谱节点
        """
        intents_file = Path(__file__).parent.parent / "data" / "intent_bridge" / "user_intents.jsonl"

        if not intents_file.exists():
            logger.debug(f"对话意图文件不存在: {intents_file}")
            return

        try:
            current_timestamp = current_time.timestamp()
            intent_states = {}  # 追踪意图状态，用于关联问题和响应
            ...
            with open(intents_file, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    try:
                        ...
                        logger.debug(f"解析意图行失败: {e}")
                        continue

            logger.debug(f"📝 从对话意图收集了 {len([n for n in self.node_index.values() if n.get('metadata', {}).get('source') in ['user_intent', 'system_response']])} 个对话节点")

        except Exception as e:
            logger.error(f"收集对话节点失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _classify_intent_type(self, raw_input: str, state: str, execution_result: str = None, error_message: str = None) -> str:
        """
        🆕 根据输入内容分类意图类型
        返回: 'question', 'doubt', 'request', 'feedback', 'conversation'
        """
        # 疑问模式检测
        doubt_patterns = ['疑问', '需要我', '帮助吗', '有什么', '是否', '能不能', '为什么', '如何']
        if any(pattern in raw_input for pattern in doubt_patterns):
            return 'doubt'

        # 问题模式检测
        question_patterns = ['？', '?', '吗', '呢', '什么', '怎么', '如何', '为什么', '是否', '能不能']
        if any(pattern in raw_input for pattern in question_patterns):
            return 'question'

        # 反馈模式
        feedback_patterns = ['错误', '失败', '正确', '好的', '同意', '确认']
    ...
    def _detect_question_pattern(self, text: str) -> str:
        """
        🆕 检测问题的语义模式
        返回: 'inquiry', 'confirmation', 'clarification', 'philosophical', 'technical'
        """
        inquiry_patterns = ['什么', '如何', '怎么', '为什么', '哪些', '哪个']
        if any(pattern in text for pattern in inquiry_patterns):
            return 'inquiry'

        confirmation_patterns = ['是否', '能不能', '可以吗', '确认', '同意']
        if any(pattern in text for pattern in confirmation_patterns):
            return 'confirmation'

        philosophical_patterns = ['智能', '意识', '本质', '意义', '存在', '思维']
        if any(pattern in text for pattern in philosophical_patterns):
            return 'philosophical'

        technical_patterns = ['代码', '文件', '修复', '错误', '系统', '功能']
    ...
    def _prune_old_nodes(self, max_nodes: int = 50000):
        """修剪最古老的历史节点"""
        # 按last_seen_timestamp排序，移除最古老的HISTORY层节点
        history_nodes = [
            (node_id, node)
            for node_id, node in self.node_index.items()
        ...
        logger.debug(f"修剪了 {nodes_to_remove} 个历史节点")

    def _collect_links(self, agi_engine) -> List[Dict[str, Any]]:
        """收集边（关系）数据"""
        links = []
        # 获取当前所有节点ID的集合，用于验证边的有效性
        valid_node_ids = set(self.node_index.keys())

        # 1. 从 TopologicalMemoryCore 提取拓扑连接（仅保留连接到有效节点的边）
        ...
        # 2. 从 BiologicalMemorySystem 提取生物记忆拓扑（仅保留连接到有效节点的边）
        ...
        # 3. 从 Knowledge Graph (NetworkX) 提取概念关系（主要来源）
        ...
        logger.info(f"📊 提取了 {len(links)} 条边 (拓扑连接: {len([l for l in links if l.get('type') == 'topological'])}, "
                   f"生物记忆: {len([l for l in links if l.get('type') == 'biological'])}, "
                   f"知识图谱: {len([l for l in links if l.get('type') == 'knowledge_graph'])}, "
                   f"🆕 对话链接: {len([l for l in links if l.get('relation') == 'answered_by'])})")
